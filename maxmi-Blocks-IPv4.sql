drop table if exists `wmt-inkiru-ds.MAXMIND.GeoIP2_Enterprise_Blocks_IPv4`;
CREATE EXTERNAL TABLE `wmt-inkiru-ds.MAXMIND.GeoIP2_Enterprise_Blocks_IPv4` (
`network` string, 
`geoname_id` string, 
...
OPTIONS (
  ...
  skip_leading_rows = 1,
  field_delimiter = ',',
  uris = ['gs://inkiru-ds-maxmind/GeoIP2-Enterprise-CSV_XXXXXXXX/GeoIP2-Enterprise-Blocks-IPv4.csv']
);
